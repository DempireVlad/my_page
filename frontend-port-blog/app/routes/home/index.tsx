...
export async function loader({
  ...
  const [projectsRes, postsRes] = await Promise.all([
    fetch(
      `${import.meta.env.VITE_API_URL}/projects?filters[featured][$eq]=true&populate=*&sort[0]=date:desc`,
    ),
    fetch(
      new URL(`${import.meta.env.VITE_API_URL}/posts?populate=*`, request.url)
        .href,
    ),
  ]);

  if (!projectsRes.ok) {
    throw new Response("Помилка завантаження проектів", {
      status: projectsRes.status,
    });
  }
  if (!postsRes.ok) {
    throw new Response("Помилка завантаження постів", {
